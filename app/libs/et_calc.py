...
# deffao_etr_hourly = function(lat, lon, J, hour, z, RH, Temp_C, Rs, P, U){
#   ########################################
...
#   w = (pi/12) * (((t + (0.06667 * (lz - lon)))+Sc)-12)
...
#   Gsc = 0.0820
#   Ra = ((12*60)/pi)*(Gsc*dr*(((w2 - w1) * sin(j) * sin(d)) + (cos(j)*cos(d)*(sin(w2) - sin(w1)))))
...
#     rel_Rs = 1
#   }
#   Rnl = 2.041308e-10 * ((Temp_C+273.16)^4)*(0.34-(0.14*(sqrt(ea))))*((1.35*(rel_Rs))-0.35)
...
# fao_etr_daily = function(lat, J, z, RH, Temp_C, Rs, P, U){
#   ########################################
#   ####### Reference ET  from FAO #########
#   ########################################
#   #
...
#   Gsc = 0.0820
#   Ra = ((24*60)/pi)*(Gsc*(ws*sin(j)*sin(d) + cos(j)*cos(d)*sin(ws)))
...
#     rel_Rs = 1
#   }
#   Rnl = 4.903e-9 * ((Temp_C+273.16)^4)*(0.34-(0.14*(sqrt(ea))))*((1.35*(rel_Rs))-0.35)
#   # incoming net shortwave radiation (Rns) assuming grass albedo (0.23)
#   Rns =  (1 - 0.23) * Rs_MJ 
#   # Compute Rn (difference between Rns and Rnl)
#   Rn = Rns - Rnl
