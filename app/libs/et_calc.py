...
def fao_etr_hourly(lat, lon, J, hour, z, RH, Temp_C, Rs, P, U):
  ...
  Rnl = 2.041308e-10 * ((Temp_C+273.16)^4)*(0.34-(0.14*(np.sqrt(ea))))*((1.35*(rel_Rs))-0.35)
  ...
  D = (4098 * (0.6108*np.exp((17.27*Temp_C)/(Temp_C + 237.3)))) / ((Temp_C + 237.3)^2)
  ...
  etr = ((0.408*D*(Rn - G)) + (g*(37/(Temp_C + 273))) * (U * (es - ea))) / (D + g*(1+0.24*U)) 
...
def fao_etr_daily(lat, J, z, RH, Temp_C, Rs, P, U):
  ...
  Gsc = 0.0820
  Ra = ((24*60)/np.pi)*(Gsc*(ws*np.sin(j)*np.sin(d) + np.cos(j)*np.cos(d)*np.sin(ws)))
  ...
  Rnl = 4.903e-9 * ((Temp_C+273.16)^4)*(0.34-(0.14*(np.sqrt(ea))))*((1.35*(rel_Rs))-0.35)
  # incoming net shortwave radiation (Rns) assuming grass albedo (0.23)
  Rns =  (1 - 0.23) * Rs_MJ 
  # Compute Rn (difference between Rns and Rnl)
  Rn = Rns - Rnl
  ...
  G = 0
  # compute etr using Eq. 53
  etr = ((0.408*D*(Rn - G)) + (g*(900/(Temp_C + 273))) * (U * (es - ea))) / (D + g*(1+0.34*U))
  #return the result
  return etr