...
def plot_wind(wind_data):
    ...
    wind_data = wind_data.with_columns(
        pl.col("Wind Direction [deg]").map_elements(deg_to_compass, return_dtype=pl.String)
    )
    
    # Round wind speed
    wind_data = wind_data.with_columns(
        pl.col("Wind Speed [mi/h]").round(0)
    )
    # Create a list of 8 whole numbers between 0 and the max wind speed
    max_speed = int(wind_data["Wind Speed [mi/h]"].max())
    speed_bins = [round(x) for x in pl.Series(range(8)).map_elements(lambda i: int(i * max_speed / 7), return_dtype=pl.Int16)]
    ...
    out = (
        wind_data
        .group_by(["Wind Direction [deg]", "Wind Speed [mi/h]"])
        .len()
    ...
    if missing_dirs and speeds:
        ...
        rows = pl.DataFrame(rows_data).cast({"Wind Speed [mi/h]": pl.Categorical, "Frequency": pl.UInt32})
        out = pl.concat([out, rows])
    
    # Convert wind direction to categorical and sort
    out = out.with_columns(
        pl.col("Wind Direction [deg]").cast(pl.Categorical)
    )
